...
export async function onRequest({ request }) {
  try {
    ...
    const text = (url.searchParams.get('text') || '').replace(/[^一-龥a-zA-Z0-9，。！？、：；]/g, '').slice(0, 150);
    ...
    const chunks = [];  // 存 Uint8Array 原始字节
      ...
      ws.onmessage = (e) => {
        ...
        try {
          ...
          if (msg.code !== 0) {
            settled = true;
            clearTimeout(timer);
            reject(new Error('iFlytek error ' + msg.code + ': ' + (msg.message || 'unknown')));
            return;
          }
          // 收集所有音频分段（status 1 中间段 + status 2 最终段）
          // 每段 base64 → 二进制解码，避免拼接时 padding 错位导致音频断裂
          if (msg.data?.audio) {
            const binStr = atob(msg.data.audio);
          ...
          settled = true;
          clearTimeout(timer);
          reject(new Error('JSON parse: ' + e.data.substring(0, 100)));
        }
      };
...
// 合并多个 Uint8Array 原始音频块
function mergeChunks(chunks) {
  const totalLen = chunks.reduce((sum, a) => sum + a.length, 0);
